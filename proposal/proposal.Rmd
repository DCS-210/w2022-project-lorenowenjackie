...
fig3 <- ggplot(data = fossildata, mapping = aes(x = year, 
...
```{r wind-consumption-all_countries-plot, message = FALSE, echo = FALSE, warning = FALSE}
wind_consumption_data <- cleandata %>%
  filter(year >= 2000) %>%
  select(wind_consumption, year, country) 

fig10 <- ggplot(data = wind_consumption_data, mapping = aes(x = year, 
                                        y = wind_consumption, 
                                        color = country))+
  stat_smooth(geom='line', alpha=0.5, se=FALSE) + 
  theme_minimal() + 
...
```{r solar-consumption-all-plot, echo = FALSE, message = FALSE, warning = FALSE}
