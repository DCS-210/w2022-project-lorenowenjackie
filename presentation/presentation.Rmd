...
```{r example-leaflet}
bins <- seq(from = 0, to = 30000, by = 5000)
pal74 <- colorBin("RdYlGn", domain = testleaflet$renewables_energy_per_capita, bins = bins) 

m <- leaflet(data = testleaflet) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(fillColor = ~pal74(testleaflet$renewables_energy_per_capita))
