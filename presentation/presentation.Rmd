...
```{r example-leaflet}
bins <- seq(from = 0, to = 40000, by = 5000)
pal <- colorBin("YlOrRd", domain = mostrecentdata$fossil_fuel_consumption, bins = bins)

m <- leaflet(data = mostrecentdata) %>% 
  addTiles()  %>% 
  setView( lat=10, lng=0 , zoom=2) %>%
  addPolygons(fillColor = ~pal(mostrecentdata$fossil_fuel_consumption))
