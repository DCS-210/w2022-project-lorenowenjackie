...
```{r bar-chart-percents}
barchartcons <- alldata %>%
  filter(country %in% c("United States", "Canada", "United Kingdom", "Spain", "France", "Germany", "Switzerland", "Italy", "Norway", "Sweden", "Finland", "China", "Brazil", "India", "Russia", "Japan", "New Zealand", "Australia", "Africa", "South Africa"), year >= 1990) %>%
  select(country,
         year,
         wind_share_energy, 
...
barchartcons <- transform(barchartcons, overall_consumption = (wind_share_energy + 
         solar_share_energy + 
         nuclear_share_energy+ 
         hydro_share_energy+
         biofuel_share_energy+
...
barchartedited <- gather(barchartcons, key = "energy_type", value = "consumption", -country, -year, -overall_consumption)

barchartcons <- barchartedited[order(-barchartedited$overall_consumption),]

barchartedited$country <- factor(barchartedited$country, 
 levels = unique(barchartedited$country))
  
colnames(barchartedited)
```
