...
.pull-right[
```{r climatechange, echo = FALSE, out.width = "80%", fig.align= "right"}
...
```{r linear-regression-fossil-fuels, message = FALSE, warning = FALSE}
m_USA <- linear_reg() %>%
  set_engine("lm") %>%
...
---
class: left, top
### Who is produced the most carbon emissions from electricity in 2018? 

```{r carbon-emissions, echo = FALSE, out.height="50%"}
pal2 <- colorNumeric(palette = "YlOrRd", domain = testleaflet$carbon_intensity_elec)
labels <- sprintf("<strong>%s</strong><br/>%s ", testleaflet$country, testleaflet$carbon_intensity_elec) %>%
  lapply(htmltools::HTML)

q <- leaflet(data = testleaflet) %>% 
  addTiles()  %>% 
  setView( lat=53, lng=9 , zoom=3) %>%
  addPolygons(fillColor = ~pal2(testleaflet$carbon_intensity_elec),
                                 fillOpacity = 1, 
                                 color = "white",
                                 opacity = 0.7,
                                 weight = 1,
                                 label = labels) %>%
  addLegend("bottomleft", pal = pal2, values = ~testleaflet$carbon_intensity_elec,
...
```{r wind-consumption-all_countries-plot, include = FALSE, echo=FALSE, out.width = "50%", out.height = "50%", message=FALSE, warning=FALSE}
...
windcons <- ggplotly(wind_consumption_graph)
```

```{r wind-consumption-per-capita-all_countries-plot, out.width = "50%", out.height = "50%", echo = FALSE, fig.alt= "Line graph of wind consumption per capita per country per year."}
...
windcap <- ggplotly(wind_energy_per_capita_graph)

wind_energy_per_capita_graph + wind_consumption_graph
...
---
class: center, middle
background-image:url(https://multifiles.pressherald.com/uploads/sites/10/2019/10/10872983_SJ.CITaerialFoliageSAPV.101419_1.jpg)
### Questions? 
---
