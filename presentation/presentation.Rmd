...
# Code
```{r leaflet-data, echo = FALSE, message = FALSE, warning = FALSE}
...
```{r 2018-data-leaflet, echo = FALSE, message = FALSE, warning = FALSE}
testleaflet <- leaflettdata %>%
  filter(year == "2018")
```

```{r filter-worldspdf-hippo, echo = FALSE, message = FALSE, warning = FALSE}
...
```{r 2018-leaflet, echo = FALSE, message = FALSE, warning = FALSE}
#2018 leaflet 
...
```
---
```{r 1990-data-leaflet, echo = FALSE, message = FALSE, warning = FALSE}
ninetyleaflet <- leaflettdata %>%
  filter(year == "1990")
```

```{r filter-worldspdf, echo = FALSE, message = FALSE, warning = FALSE}
ninetyleaflet <- ninetyleaflet %>% 
  left_join(world_spdf2, by = c("country" = "NAME")) %>%
  st_as_sf()
```

```{r ninety-leaflet, echo = FALSE, message = FALSE, warning = FALSE}
...
b <- leaflet(data = ninetyleaflet) %>% 
  ...
  addPolygons(fillColor = ~pal2(ninetyleaflet$renewables_share_energy),
                                 ...
                                 weight = 1,
                                 label = labels) %>%
  addLegend("bottomright", pal = pal2, values = ~ninetyleaflet$renewables_share_energy,
...
```
---
```{r barchart-transformed-fig, echo = FALSE, message = FALSE, warning = FALSE}
barcharttransform %>%
  filter(year >= 2000) %>%
...
```
---
```{r china-usa-bar, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r time-lapse}
#multiyearfig <- ggplot(leaflettdata) + 
 # geom_sf(aes(fill = renewables_share_energy)) +
#  labs(fill = "Renewable Share of Energy") + 
...
>>>>>>> 56600d497cf474ebd7bdff36271f45bb459d5aa6
